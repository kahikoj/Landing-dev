    ...
    render() {
        return (
            ...
            <Container className="mt-100 mt-60">
                ...
                <SectionTitle title="Our Instructor" desc=" that can provide everything you need to generate awareness, drive traffic, connect."/>
                        ...
                        this.state.instructors.map((instructor, key) =>
                            <Col lg={4} md={6} xs={12} key={key} className="mt-4 pt-2">
                                <Media className="align-items-center">
                                    <img src={instructor.img} className="avatar avatar-medium rounded-circle img-thumbnail" alt="Landrick" />
                                    <div className="content ml-3">
                                        <h5 className="mb-0"><Link to="#" className="text-dark">{instructor.name}</Link></h5>
                                        <small className="position text-muted">{instructor.post}</small>
                                    </div>
                                </Media>
                            </Col>
                        )
