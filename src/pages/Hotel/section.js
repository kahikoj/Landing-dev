...
class Section extends Component {
    ...
    componentDidMount(){
        ...
        for(var i=0; i<3; i++){
            if(i===0)
                e1[i].style.backgroundImage = `url(${bg01})`;
            if(i===1)
                e1[i].style.backgroundImage = `url(${bg02})`;
            if(i===2)
                e1[i].style.backgroundImage = `url(${bg03})`;
        }
    ...
    componentWillUnmount(){
        document.getElementById("btn1").removeEventListener("click", () => this.setState({ isOpen: true }));
    }    
